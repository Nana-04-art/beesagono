...
2. **No anti-cheat guarantee** — `possibleWords`, `mielegrammi`, and score math are all inspectable/editable client-side.
...
| **Persistence** | Spring Data JPA + Hibernate, dialect-switchable via Spring profiles |
| **Databases** | MySQL (primary target) and Oracle (secondary target) — schema is written in portable ANSI SQL types to support both via Hibernate dialect configuration (`spring.jpa.database-platform`) |
...
| **Build** | Maven or Gradle (either is compatible; examples in this doc set assume Maven) |
| **API Style** | REST, JSON, versioned under `/api/v1` |

> **ID strategy:** every entity's primary key is a `VARCHAR(36)` UUID string, generated **application-side** (`UUID.randomUUID().toString()`) rather than DB auto-increment. This keeps IDs non-guessable, avoids cross-database auto-increment portability issues (MySQL vs Oracle sequence semantics differ), and lets the application assign an ID before the first `INSERT` (useful for the puzzle-generation and guest-merge flows, which sometimes need to reference an ID before the transaction commits).
...
- **Services own transactions.** Any method that writes to more than one table (e.g. `GameStateService.submitWord()`, which updates `game_states`, `game_state_found_words`, and triggers `StatsService`) is annotated `@Transactional` so the write is atomic.
...
- All PRNG logic (Mulberry32) and the `hashDateString` (djb2 variant) hashing are ported **bit-for-bit** into `PuzzleGeneratorService.java`, using Java's 32-bit `int` arithmetic and explicit unsigned-shift equivalents (`>>> 0` in TypeScript maps to using `int` with `>>>` unsigned right shift in Java, since Java `int` is natively 32-bit signed — care must be taken with sign-extension differences and verified against the frontend's own test vector, see `backend-data-model.md §4`).
- **Generation trigger:** puzzles are generated **lazily**, on the first `GET /api/v1/puzzle/today` request for a date that has no row yet in `daily_puzzles`. A scheduled job (`@Scheduled` cron, e.g. run at `00:01` local server time) may optionally pre-generate the next day's puzzle to avoid a cold-start on the first request of the day, but lazy generation remains the source of truth/fallback either way.
- Once generated, `daily_puzzles` (+ its child tables) is **immutable** for that date — regenerating it would break score determinism for anyone who already played.
...
- **Authentication:** stateless JWT.
  ...
  - **Refresh token:** long-lived (recommended default: **30 days**), opaque random string, persisted server-side (hashed) in a `refresh_tokens` table (see `backend-data-model.md`) so it can be **revoked** — this is what makes logout/invalidation actually work, since a bare JWT access token cannot be invalidated before its natural expiry without server-side state.
  ...
  - `POST /api/v1/auth/logout` revokes the refresh token (marks it `revoked = true`), so it can no longer be exchanged; the short-lived access token is left to expire naturally (standard trade-off of stateless access tokens).
...
- **Transport:** HTTPS is mandatory in all non-local environments; JWTs are never accepted over plaintext HTTP in production configuration.
...
- **Database dialect switching:** `spring.jpa.database-platform` and datasource URL are externalized per Spring profile (`application-mysql.yml`, `application-oracle.yml`), so the same JPA entities/schema target either engine without code changes, provided only portable SQL types are used (see `backend-data-model.md §1`).
...
- **Secrets** (JWT signing key, DB credentials): externalized via environment variables / a secrets manager — never committed to source control.